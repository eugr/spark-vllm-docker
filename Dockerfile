...
# Apply DeepSeek-V4 REAP GB10 native-image patches (router fallback for
# nonstandard REAP expert counts + MoE weight-loading memory hygiene)
COPY mods/deepseek-v4-reap-gb10/patch_vllm_reap_gb10.py /opt/deepseek-v4-reap-gb10/
RUN python3 /opt/deepseek-v4-reap-gb10/patch_vllm_reap_gb10.py

# Apply MiniMax-M2.7 REAP NVFP4 GB10 native-image patch (Humming FP8 lm_head
# fallback for ParallelLMHead + NVML max-clock fallback on GB10)
COPY mods/minimax-m2.7-reap-nvfp4-gb10/patch_vllm_minimax_m27.py /opt/minimax-m2.7-reap-nvfp4-gb10/
RUN python3 /opt/minimax-m2.7-reap-nvfp4-gb10/patch_vllm_minimax_m27.py

# MiniMax-M2.7 REAP layers 60/61 ship a NaN NVFP4 activation global scale;
# substitute the healthy neighbor value so the qkv_proj stops emitting NaN.
COPY mods/minimax-m2.7-reap-nvfp4-gb10/patch_nvfp4_scalefix.py /opt/minimax-m2.7-reap-nvfp4-gb10/
RUN python3 /opt/minimax-m2.7-reap-nvfp4-gb10/patch_nvfp4_scalefix.py
