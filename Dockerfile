...
RUN set -eux; \
    ...
    pip download "nvidia-nccl-cu13" -d /tmp/nccl-wheel; \
    WHL="$(ls /tmp/nccl-wheel/nvidia_nccl_cu13-*.whl | head -n1)"; \
    unzip "$WHL" -d /tmp/nccl-wheel/unpacked; \
    cp -L build/lib/libnccl.so.2 /tmp/nccl-wheel/unpacked/nvidia/nccl/lib/libnccl.so.2; \
    rm "$WHL"; \
...
# See https://github.com/vllm-project/vllm/pull/47604
ARG VLLM_PRESET_PRS="47445 47392 47604"
...
RUN --mount=type=bind,source=wheels,target=/workspace/wheels \
    ...
    echo "nvidia-nccl-cu13 @ file://${NCCL_WHEEL}" >> /tmp/wheel-override.txt && \
...
# Fix NCCL (not necessary anymore, custom wheel will be installed anyway)
