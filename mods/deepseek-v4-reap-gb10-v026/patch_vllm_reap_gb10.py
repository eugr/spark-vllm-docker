...
def main() -> int:
    ...
    # This vLLM build's fused_topk_bias_router.py may already carry a native
    # REAP nonstandard-expert-count fallback (either this patch's own "or ("
    # form, or the older "and not (" form some forks/images ship natively) —
    # in that case there is nothing to patch.
    if ROUTER_NEW in router_text or "gating_output.shape[-1] not in" in router_text:
        print(f"PATCH_ALREADY_EQUIVALENT {ROUTER_TARGET.name}")
    elif ROUTER_OLD in router_text:
        print(patch_once(ROUTER_TARGET, ROUTER_OLD, ROUTER_NEW))
    else:
        print(f"PATCH_SKIPPED_NO_ROUTER_ANCHOR {ROUTER_TARGET.name}")
