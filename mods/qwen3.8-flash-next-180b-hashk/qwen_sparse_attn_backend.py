...
def _torch_sdpa_varlen_func(
    ...
    import os as _os
    if _os.environ.get("SGLANG_QSA_SHIM_DEBUG") == "1":
        import logging
        _lg = logging.getLogger("qsa_shim")
        _lg.warning(
            "QSA shim: B=%d lens=%s qnan=%s knan=%s outnan=%s outamax=%.3f",
            B, lens[:4].tolist(), bool(torch.isnan(q).any()),
            bool(torch.isnan(kh).any()), bool(torch.isnan(out).any()),
            out.float().abs().amax().item(),
        )
    return out
...
def _resolve_flash_attn_varlen_func():
    ...
    """
    return _torch_sdpa_varlen_func
    try:
...
class QwenSparseAttnBackend(AttentionBackend):
    ...
    def _forward_paged_attention(
        ...
        # --- local patch: direct torch sparse-decode attention ---
